...
(async () => {
  ...
  function renderClasses(filter='all') {
    ...
    document.querySelector('#class-grid').innerHTML = list.length ? list.map(item=>{
      ...
      return `<article class="class-card ${item.featured?'featured':''}"><div class="card-top"><span class="class-number" aria-hidden="true">${index}</span><span class="pill">${e(item.category)}</span></div><h3>${e(item.name)}</h3><p>${e(item.description)}</p><div class="class-meta"><span>${e(item.audience)}</span><span>◷ ${e(item.schedule || 'Contact the studio for timings')}</span>${instructor?`<span>With ${e(instructor.name)}</span>`:''}<span>${e(item.fee || 'Ask for current fees')}</span></div><a class="text-link" href="${e(wa(item.name))}" target="_blank" rel="noopener noreferrer" aria-label="Enquire about ${e(item.name)} on WhatsApp">Let’s do this <span aria-hidden="true">↗</span></a></article>`;
    }).join('') : '<p class="content-loading">No classes listed in this category yet. Contact the studio for options.</p>';
  }
  renderClasses();
  document.querySelectorAll('[data-filter]').forEach(button=>button.addEventListener('click',()=>{
    document.querySelectorAll('[data-filter]').forEach(b=>{b.classList.toggle('active',b===button);b.setAttribute('aria-pressed',String(b===button));});renderClasses(button.dataset.filter);
  }));
  ...
  const revealTargets=document.querySelectorAll('.class-card,.studio-layout,.instructor-card,.team-note,.review-card,.contact-section');
  ...
  function count(element){const target=Number(element.dataset.number);const decimals=Number(element.dataset.decimals || 0);if(reduced){element.textContent=target.toFixed(decimals);return;}const start=performance.now();function tick(now){const t=Math.min(1,(now-start)/1600);element.textContent=(target*(1-Math.pow(1-t,3))).toFixed(decimals);if(t<1)requestAnimationFrame(tick);}requestAnimationFrame(tick);}
